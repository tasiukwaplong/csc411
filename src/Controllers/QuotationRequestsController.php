...
class QuotationRequestsController extends Database{
	...
	public function addQuotation($adminToken = null, $data = []){
	  // add quotation
	  if (!$this->isAdmin($adminToken)) return $this->status(true, GENREAL_MESSAGES['admin_not_auth']);
		...
		if (parent::insert($data)['errored']) return $this->status(true, GENREAL_MESSAGES['quotation_not_added'] ?? $this->status['message']);
